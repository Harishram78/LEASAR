...
6-DOF UR3 robot simulation with:
  ...
  - Gym-compatible interface for SAC/PPO/DQN training
  - Sim-to-real transfer: same obs vector as pipeline/data_acquisition.py
...
class UR3SimEnv(gym.Env):
    ...
    # -- Physics ----------------------------------------------------------------
    def _connect(self):
        mode = pb.GUI if self.render_mode else pb.DIRECT
    ...
    def _load_robot(self):
        ...
        try:
            ...
            # Fallback to KUKA iiwa if UR3 URDF not present
            robot_id = pb.loadURDF(
...
def make_ur3_env(render: bool = False, fault_prob: float = 0.15) -> UR3SimEnv:
    ...
    return UR3SimEnv(render=render, fault_prob=fault_prob)
